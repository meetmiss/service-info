  ...
  <div class="detail">
      ...
      <el-row :gutter="20">
        ...
        <el-col :span="12" v-if="ruleForm.IS_SERVICE_COMPANY === '有'">
          <el-form-item label="公司名称" prop="DATA_SERVICE_COMPANYNAME">
            <el-input v-model="ruleForm.DATA_SERVICE_COMPANYNAME"></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="12" v-if="ruleForm.IS_SERVICE_COMPANY === '有'">
        ...
        <el-col :span="12">
          <el-form-item label="备注" prop="D_MOME">
            <el-input
              ...
              placeholder="如省级单位通信方式选择专线，则需填写专线是否由信息中心统一管理；如国家级单位通信方式选择专线，则需填写专线是否由国家气象信息中心统一管理"
            ></el-input>
          </el-form-item>
        </el-col>
      </el-row>
    ...
    <div class="table">
      <el-table
        ...
        :fit="true"
      >
        <el-table-column align="center" label="校验信息" min-width="150">
        ...
        <el-table-column align="center" label="单位信息">
          <el-table-column align="center" label="单位名称" min-width="200">
            <template slot-scope="scope">
              <span v-if="scope.row.D_CHECK_INFO" style="color: #f00">{{
                scope.row.D_DEPART_NAME
              }}</span>
        ...
        <el-table-column align="center" label="服务内容">
          ...
          >
            <template slot-scope="scope">
              <span v-if="scope.row.D_CHECK_INFO" style="color: #f00">{{
                scope.row.D_DATA_TYPE | cotegoryChange
              }}</span>
        ...
        <el-table-column align="center" label="公司信息">
          <el-table-column align="center" label="公司名称" min-width="170">
            <template slot-scope="scope">
              <span v-if="scope.row.D_CHECK_INFO" style="color: #f00">{{
                scope.row.DATA_SERVICE_COMPANYNAME
              }}</span>
        ...
        >
          <template slot-scope="scope">
            <span v-if="scope.row.D_CHECK_INFO" style="color: #f00">{{
              scope.row.DATA_SERVICE_CONDITION
            }}</span>
...
export default {
  ...
  data () {
    return {
      ...
      ruleForm: {
        D_CHECK_INFO: "", // 校验信息
        D_DEPART_NAME:
          this.userInfo && this.userInfo.depart ? this.userInfo.depart : "", // 单位名称
        D_DEPART_LEVEL: "", // 单位类别
        D_SERVICE_MODE: "", // 通信方式
  ...
  methods: {
        ...
        .then((res) => {
          ...
          if (res.data.retcode === "01") {
            res.data.data.forEach((item) => {
              item.D_DATA_TYPE =
                item.D_DATA_TYPE && item.D_DATA_TYPE.split(",");
            ...
            this.total = res.data.count[0].NUM;
            // this.showTableData = [];
    ...
    handleExport () {
      ...
      window.open(
        href +
          "/MDRSC/api/rest/ExcelDownloadServlet.svt?TABLE_NAME=ESMD_INFO_DETAIL"
      );
    },
    // 清空
    handleClearData () {
